...
- **Embeddable orchestration core** — `Agent` (built via `AgentBuilder`) owns the long-lived state (session, `ProviderRegistry`, `ToolRegistry`, `AgentConfig`, cancel token) and is the composable entry point for every consumer — the `rho` binary, embedders, tests, benches. `run()` / `run_turn()` drive `run_loop`; per-turn observer / approval-gate / steering / cancel arrive via `TurnInputs`. Construction is decoupled from CLI, config files, and stdio (see [Embedding `rho-core`](#embedding-rho-core)).
...
**Startup sequence (`App::build`):** `App` is a thin host shell around a `rho_core::Agent`; `App::build` maps the `Cli` into an `AgentBuilder` (which assembles the kernel `Agent` — providers, model, budget, session) and attaches the host-only concerns below.
