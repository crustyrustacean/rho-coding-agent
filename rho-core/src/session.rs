...
impl Session {
    ...
    pub async fn compact_older_than(
        ...
        for (i, entry) in chronological.iter().enumerate().skip(1) {
            ...
            cumulative_tokens +=
                truncation::estimate_entry_tokens_for_compaction(entry, self.estimator.as_ref());
            compact_end = i + 1;
    ...
    pub fn append_tool_result(
        ...
        let max_tokens = (self.token_budget.prompt_budget() as f32
            * truncation::MAX_TOOL_RESULT_FRACTION) as usize;
        ...
        let (content, details) = if estimated_tokens > max_tokens {
            ...
            let max_chars =
                truncation::chars_to_fit_tokens(&redacted, max_tokens, self.estimator.as_ref());
            let original_size = redacted.len();
            let truncated = format!(
                "{}\n\n{}",
                &redacted[..truncation::floor_char_boundary(&redacted, max_chars)],
                truncation::truncation_footer(original_size),
            );
