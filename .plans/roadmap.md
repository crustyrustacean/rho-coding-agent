...
- **Context window management** — A `ContextManager` trait keeps the conversation within the model's context limit. The default `SlidingWindowContextManager` pins the system message and evicts by *turn*, never splitting an assistant `tool_calls` message from its matching `tool` results. The trait boundary lets future strategies (summarisation, retrieval-augmented context) swap in without changing the agent loop. The pinning and no-split-tool-pairs invariants must be preserved by any future strategy. The token budget defaults to 32K (raised from 8K in Phase 2) and is configurable via `[agent] token_budget` and `--token-budget`. Phase 4 adds model-aware sizing (query the API for context window length); Phase 5 adds budget-aware prompt composition (measure system prompt cost, warn when it consumes too much of the budget).
