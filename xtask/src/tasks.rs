...
pub fn generate_models() -> Result<()> {
    ...
    let overrides_str =
        std::fs::read_to_string(&overrides_path).with_context(|| format!("failed to read {overrides_path}"))?;
...
//! Overrides: rho-ai/model-overrides.json\n\n",
    ...
    for m in models {
        ...
        let cost_cache_read = cache_read_per_token
            .map(|v| v * 1_000_000.0)
            .unwrap_or(0.0);
        let cost_cache_write = cache_write_per_token
            .map(|v| v * 1_000_000.0)
            .unwrap_or(0.0);
        ...
        buf.push_str("        Model {\n");
        buf.push_str(&format!("            id: {:?}.to_string(),\n", id));
        buf.push_str(&format!("            name: {:?}.to_string(),\n", name));
        buf.push_str(&format!("            provider: {:?}.to_string(),\n", provider));
        buf.push_str(&format!("            context_window: {},\n", context_length));
        buf.push_str(&format!("            max_tokens: {},\n", max_tokens));
        buf.push_str(&format!("            input: ModelInput {{ text: {}, image: {} }},\n", text, image));
        buf.push_str("            cost: ModelCost {\n");
        buf.push_str(&format!("                input: {:.10},\n", cost_input));
        buf.push_str(&format!("                output: {:.10},\n", cost_output));
        buf.push_str(&format!("                cache_read: {:.10},\n", cost_cache_read));
        buf.push_str(&format!("                cache_write: {:.10},\n", cost_cache_write));
        buf.push_str("            },\n");
        buf.push_str("            thinking: ModelThinking {\n");
        buf.push_str(&format!("                supported: {},\n", thinking_supported));
